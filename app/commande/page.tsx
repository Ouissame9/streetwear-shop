...
export default function Commande() {
  ...
  const handleCommander = () => {
    if (!nom || !prenom || !telephone || !adresse || !ville || !codePostal) { alert("Remplis tous les champs !"); return; }
    if (!/^(0[67][0-9]{8})$/.test(telephone)) { alert("Numéro de téléphone invalide — Ex: 0698242804"); return; }
    if (!/^[0-9]+\s+.{3,}$/.test(adresse)) { alert("Adresse invalide — Ex: 12 Rue de la Paix"); return; }
    if (!/^[0-9]{5}$/.test(codePostal)) { alert("Code postal invalide — 5 chiffres requis"); return; }
    if (!/^[a-zA-ZÀ-ÿ\s\-]{2,}$/.test(ville)) { alert("Ville invalide"); return; }
    ...
    <main style={{minHeight: "100vh", background: "black", color: "white"}}>
      <nav style={{display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 16px", borderBottom: "1px solid rgba(255,255,255,0.1)", position: "sticky", top: 0, zIndex: 50, background: "rgba(0,0,0,0.92)", backdropFilter: "blur(10px)", width: "100%", boxSizing: "border-box"}}>
        ...
        <div style={{display: "flex", gap: "10px", alignItems: "center"}}>
          <a href="/panier" style={{border: "2px solid #4ade80", color: "#4ade80", background: "transparent", fontWeight: 900, padding: "8px 12px", borderRadius: "999px", fontSize: "12px", textDecoration: "none"}}>🛒</a>
      ...
      <section style={{maxWidth: "600px", margin: "0 auto", padding: "40px 20px"}}>
              ...
              {panier.map((p, i) => (
                <div key={i} style={{display: "flex", gap: "12px", alignItems: "center", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "12px", padding: "12px", background: "rgba(255,255,255,0.02)"}}>
                  <div style={{width: "48px", height: "48px", borderRadius: "8px", overflow: "hidden", flexShrink: 0, background: "#111"}}>
                    {p.photo ? <img src={p.photo} alt={p.name} style={{width: "100%", height: "100%", objectFit: "cover"}} /> : <div style={{width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "22px"}}>👕</div>}
                  </div>
        ...
        <div style={{display: "flex", flexDirection: "column", gap: "14px", marginBottom: "24px"}}>
          {[
            ...
            <div key={field.label}>
              ...
              <input value={field.value} onChange={e => field.setter(e.target.value)} placeholder={field.placeholder} type={field.type} maxLength={field.maxLength}
                style={{width: "100%", background: "rgba(255,255,255,0.05)", border: `1px solid ${field.value.length === 0 ? "rgba(255,255,255,0.1)" : field.pattern && !new RegExp(field.pattern).test(field.value) ? "rgba(255,80,80,0.6)" : "rgba(74,222,128,0.4)"}`, borderRadius: "12px", padding: "14px 16px", color: "white", fontSize: "15px", outline: "none", boxSizing: "border-box"}} />
              {field.value.length > 0 && field.pattern && !new RegExp(field.pattern).test(field.value) && (
                <p style={{color: "rgba(255,80,80,0.8)", fontSize: "11px", margin: "4px 0 0", fontWeight: 700}}>
                  ...
                  {field.label === "Ville" && "Ville invalide — lettres uniquement"}
                </p>
              )}
            </div>
          ))}
        ...
        {panier.length === 0 ? (
          ...
          <>
            <button onClick={handleCommander} style={{width: "100%", background: "#4ade80", color: "black", fontWeight: 900, padding: "18px", borderRadius: "999px", fontSize: "16px", border: "none", cursor: "pointer", boxShadow: "0 0 30px rgba(74,222,128,0.4)", marginBottom: "12px"}}>
              ENVOYER SUR WHATSAPP 📱
            </button>
            <p style={{textAlign: "center", color: "rgba(255,255,255,0.3)", fontSize: "12px"}}>Tu vas être redirigé vers WhatsApp avec le récap</p>
          </>
        )}
      </section>
